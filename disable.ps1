##################################################
# HelloID-Conn-Prov-Target-Salesforce-SCIM-Disable
# PowerShell V2
##################################################
...
try {
    ...
    switch ($action) {
        'DisableAccount' {
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Disabling Salesforce-SCIM account with accountReference: [$($actionContext.References.Account)]"
                ...
                Write-Information "[DryRun] Disable Salesforce-SCIM account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditLogMessage = "Could not disable Salesforce-SCIM account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    }
    else {
        $auditLogMessage = "Could not disable Salesforce-SCIM account. Error: $($_.Exception.Message)"
